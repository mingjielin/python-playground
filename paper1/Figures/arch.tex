...
\usetikzlibrary{positioning} % Important for the 'of' syntax
...
\tikzstyle{sensor}=[draw, fill=blue!20, text width=10em, 
    text centered, minimum height=2.5em]
\tikzstyle{ann} = [above, text width=10em]
\tikzstyle{naveqs} = [sensor, text width=8em, fill=red!30, 
...
\begin{tikzpicture}
    ...
    \node (naveq) [naveqs] {\textbf{Bidirectional Diffusion Module} $\Psi$};

    \path (naveq.east)+(-\blockdist,0) node (gyros) [sensor] 
        {\textbf{Sequence Transformer Encoder}$\phi_T$};
    \path (naveq.-150)+(-\blockdist,0) node (accel) [sensor] 
        {\textbf{Geometric GNN Encoder} $\phi_G$};
    
    \draw[->] (gyros.west)+(-2, 0) -- (gyros.west) 
    node[midway, above] {Sequence \par $\mathbf{s}$};

    \draw[->] (accel.west)+(-2, 0) -- (accel.west) 
