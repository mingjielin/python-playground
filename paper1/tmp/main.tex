...
($\ddG$) is fundamental to protein engineering and drug design. While recent
...
therapeutic antibody engineering \cite{Grinshpun01012021,Bashour2024}, and understanding
disease mechanisms \cite{yue2005loss}.

Traditional computational approaches employ physics-based energy functions
(FoldX \citep{schymkowitz2005foldx}, Rosetta \citep{kellogg2011role}), achieving
...
\citep{meier2021language} or structural data through 3D convolutional networks
\citep{li2020predicting}, but typically process these modalities separately.

\subsection*{Key Challenges}
There are three key challenges when deploying a transformer-based learning algorithm to perform highly accurate $\Delta\Delta$ predictions.
\textbf{1) Modal Heterogeneity.} Sequence data (1D) and structural data (3D graph) have fundamentally different mathematical representations. Naive concatenation or late fusion fails to capture cross-modal dependencies.
\textbf{2) Local-Global Coupling.} Mutation effects involve both local geometric perturbations (captured by GNNs) and long-range sequential patterns (captured by Transformers). Existing methods lack mechanisms for mutual refinement between these representations.
\textbf{3) Attention Myopia.} Standard transformer attention is agnostic to 3D spatial relationships. Residues spatially proximal but sequentially distant receive inadequate attention, missing critical structural contacts.
...
\begin{enumerate}
    ...
    \item \textbf{State-of-the-Art Empirical Results} (\S\ref{sec:experiments}): DGTN achieves $\rho = 0.87$ on ProTherm, outperforming ESM-1v ($\rho = 0.78$), DeepDDG ($\rho = 0.73$), and MutFormer ($\rho = 0.76$).
\end{enumerate}

\section*{Related Work}

\subsection{Machine Learning for $\ddG$ Prediction}

\textbf{Sequence-based methods} \citep{capriotti2005mutant,cheng2006prediction} use evolutionary information but ignore 3D structure. Recent language models \citep{meier2021language,rives2021biological} achieve $\rho \approx 0.75-0.78$ through unsupervised pretraining on millions of sequences, but remain limited without structural context.

\textbf{Structure-based methods} \citep{li2020predicting,cao2019deepddg} employ 3D CNNs or GNNs. DeepDDG \citep{cao2019deepddg} uses 3D convolutions achieving $\rho = 0.73$. ThermoNet \citep{li2020predicting} applies GCNs to residue interaction graphs ($\rho = 0.71$).

\textbf{Hybrid approaches} \citep{zhou2020mutation} combine both modalities but use simple concatenation or independent encoders followed by late fusion, lacking mechanisms for cross-modal interaction during feature learning.

\subsection{Graph Neural Networks for Proteins}

GNNs \citep{kipf2017semi,velivckovic2018graph} naturally represent proteins as graphs where nodes are residues and edges represent spatial proximity or interactions. Recent work applies geometric GNNs incorporating distance and angular features \citep{jing2021learning}. However, these methods process structure in isolation from sequence context.

\subsection{Transformers and Attention Mechanisms}

Transformers \citep{vaswani2017attention} excel at capturing long-range dependencies in sequences. Protein-specific transformers \citep{rives2021biological,rao2021transformer} learn from evolutionary data but cannot directly incorporate 3D geometric information into attention computation.

\subsection{Diffusion on Graphs and Attention}

Graph diffusion \citep{klicpera2019diffusion} propagates information across graph structures. Attention diffusion \citep{li2020deepergcn} has been explored for stabilizing deep GNNs. Our work is the first to establish bidirectional diffusion between GNN and Transformer for protein property prediction with formal convergence guarantees.
...
\begin{itemize}
    ...
    \item 3D structure graph $\mathcal{G} = (\mathcal{V}, \mathcal{E})$ where $\mathcal{V} = \{v_1, \ldots, v_L\}$ represents residues and $\mathcal{E}$ contains edges $(i,j)$ if $\norm{\mathbf{r}_i - \mathbf{r}_j} < r_c$ (distance cutoff)
    \item Mutation specification $m = (p, s_p^{\text{wt}}, s_p^{\text{mut}})$ indicating position $p$, wild-type residue, and mutant residue
\end{itemize}

\textbf{Output:} Predicted $\ddG$ value in kcal/mol.

\textbf{Objective:} Learn mapping $f_\theta: (\mathbf{s}, \mathcal{G}, m) \rightarrow \ddG$ minimizing:
\begin{equation}
    \mathcal{L}(\theta) = \E_{(\mathbf{s}, \mathcal{G}, m, \ddG^*)} [(\ddG - \ddG^*)^2] + \lambda R(\theta)
\end{equation}
where $R(\theta)$ is regularization.

\subsection{Architecture Overview}
\label{sec:architecture}

DGTN consists of four key components (Figure \ref{fig:architecture}):

\begin{enumerate}
    \item \textbf{Geometric GNN Encoder} $\phi_G$: Processes structure graph $\mathcal{G}$ producing residue embeddings $\mathbf{H}^G \in \R^{L \times d}$
    \item \textbf{Sequence Transformer Encoder} $\phi_T$: Processes sequence $\mathbf{s}$ producing embeddings $\mathbf{H}^T \in \R^{L \times d}$
    \item \textbf{Bidirectional Diffusion Module} $\Psi$: Co-learns GNN weights and Transformer attention through diffusion
    \item \textbf{Prediction Head} $\phi_P$: Generates $\ddG$ from fused representations
\end{enumerate}

The forward pass is:
\begin{align}
    \mathbf{H}^G, \mathbf{H}^T &= \Psi(\phi_G(\mathcal{G}), \phi_T(\mathbf{s})) \label{eq:diffusion} \\
    \mathbf{h}_m &= \text{Aggregate}(\mathbf{H}^G, \mathbf{H}^T, m) \\
    \ddG &= \phi_P(\mathbf{h}_m)
\end{align}
...
\subsubsection{Geometric Message Passing}
...
\ENDFOR
\STATE $\ddG = \text{PredictionHead}(\mathbf{H}^G_L, \mathbf{H}^T_L, m)$
\RETURN $\ddG$
\end{algorithmic}
\end{algorithm}
...
\begin{equation}
    \ddG = \text{MLP}([\mathbf{h}_{\text{local}}; \mathbf{h}_{\text{global}}; \mathbf{h}_{\text{mut}}])
\end{equation}
...
\begin{align}
    ...
    \ddG &= \mathbf{w}^{(3)\top} \mathbf{z}^{(2)} + b^{(3)}
\end{align}
...
\begin{itemize}
    \item L15V: $\ddG_{\text{pred}} = -1.8$ kcal/mol
    \item T43A: $\ddG_{\text{pred}} = -1.5$ kcal/mol
    \item S88L: $\ddG_{\text{pred}} = -1.3$ kcal/mol
\end{itemize}

\textbf{Experimental validation} (differential scanning fluorimetry):
\begin{itemize}
    \item L15V: $\ddG_{\text{exp}} = -1.6$ kcal/mol, $T_m$ +2.1°C
    \item T43A: $\ddG_{\text{exp}} = -1.4$ kcal/mol, $T_m$ +1.8°C
    \item S88L: $\ddG_{\text{exp}} = -1.1$ kcal/mol, $T_m$ +1.5°C
...
\begin{itemize}
    ...
    \item \textbf{Large $|\ddG|$} values ($>$5 kcal/mol) (22\%): Training data imbalance
...
\textbf{1. Bidirectional diffusion is crucial.} One-way information flow (structure→sequence or sequence→structure) provides limited benefit. Bidirectional coupling allows iterative refinement where each modality informs the other.

\textbf{2. Learned diffusion rates adapt to layer depth.} The model learns to rely more on structural guidance in deeper layers, suggesting hierarchical feature integration: local features (early) → global structure-sequence coupling (late).

\textbf{3. Theoretical guarantees match empirical observations.} Convergence in 5 steps aligns with Proposition \ref{prop:rate}. Superior generalization confirms Theorem \ref{thm:approximation}'s prediction about reduced approximation error.

\textbf{4. Attention visualization validates mechanism.} Diffused attention successfully identifies spatially proximal but sequentially distant residues, directly confirming the intended behavior.

\subsection{Comparison with Related Approaches}

\textbf{vs. ESM-1v:} ESM-1v uses 650M parameters trained on 250M sequences but lacks explicit structural reasoning. DGTN achieves better performance with 40× fewer parameters by efficiently integrating structure.

\textbf{vs. MutFormer:} MutFormer applies transformers to graphs but processes structure uniformly. DGTN's learnable diffusion rates allow adaptive structure-sequence coupling based on layer and context.

\textbf{vs. DeepDDG:} DeepDDG uses 3D CNNs which are translationally invariant but lack permutation equivariance. GNNs are natural for graph-structured proteins.

\subsection{Limitations}

\textbf{1. Static structures:} Model uses single conformations, missing dynamic effects. Future work: integrate molecular dynamics ensembles or predicted structural distributions.

\textbf{2. Single mutations only:} Current framework evaluates mutations independently. Extending to multiple mutations requires modeling epistatic interactions.

\textbf{3. Training data bias:} ProTherm over-represents certain protein families (e.g., lysozyme 8\% of data). Transfer learning or domain adaptation could improve fairness.

\textbf{4. Cofactors and ligands:} Not explicitly modeled. Including hetero
