...
\textbf{Sequence-based methods} \citep{capriotti2005mutant,cheng2006prediction}
use evolutionary information but ignore 3D structure. Recent language models
\citep{meier2021language,rives2021biological} achieve $\rho \approx 0.75-0.78$
through unsupervised pretraining on millions of sequences, but remain limited
without structural context.

\textbf{Structure-based methods} \citep{li2020predicting,cao2019deepddg} employ
3D CNNs or GNNs. DeepDDG \citep{cao2019deepddg} uses 3D convolutions achieving
$\rho = 0.73$. ThermoNet \citep{li2020predicting} applies GCNs to residue
interaction graphs ($\rho = 0.71$).

\textbf{Hybrid approaches} \citep{zhou2020mutation} combine both modalities but
use simple concatenation or independent encoders followed by late fusion,
lacking mechanisms for cross-modal interaction during feature learning.

\subsection{Graph Neural Networks for Proteins}

GNNs \citep{kipf2017semi,velivckovic2018graph} naturally represent proteins as
...
angular features \citep{jing2021learning}. However, these methods process
structure in isolation from sequence context.

\subsection{Transformers and Attention Mechanisms}

Transformers \citep{vaswani2017attention} excel at capturing long-range
dependencies in sequences. Protein-specific transformers
\citep{rives2021biological,rao2021transformer} learn from evolutionary data but
cannot directly incorporate 3D geometric information into attention
computation.

\subsection{Diffusion on Graphs and Attention}

Graph diffusion \citep{klicpera2019diffusion} propagates information across
graph structures. Attention diffusion \citep{li2020deepergcn} has been explored
