...
def focal_loss_for_outliers(predictions, targets, train_idx=None, alpha=1.0, gamma=2.0):
    ...
    focal_weight = torch.pow(1 - torch.exp(-base_loss), gamma)
    focal_loss = alpha * focal_weight * base_loss
...
#     def train_epoch_with_analysis(self, optimizer, criterion_type='huber'):
#         """
#         Train one epoch with loss analysis
#         """
#         self.model.train()
#         total_loss = 0
#         all_predictions = []
...
#                 loss = self.huber_loss_with_weighting(
#                     predictions, labels, 
#                     batch.get('sample_indices')  # If you track sample indices
#                 )
#             elif criterion_type == 'focal':
#                 loss = self.focal_loss_for_outliers(predictions, labels)
#             else:
...
#                 all_sample_indices.extend(batch['sample_indices'].cpu().numpy())
#         
#         # Analyze losses for this epoch
#         pred_tensor = torch.tensor(all_predictions)
#         target_tensor = torch.tensor(all_targets)
#         epoch_losses = (pred_tensor - target_tensor) ** 2
#         
#         # Update sample weights based on loss history
#         if all_sample_indices:
#             for i, idx in enumerate(all_sample_indices):
#                 loss_val = epoch_losses[i].item()
#                 self.sample_weights[idx] = self.sample_weights.get(idx, 0) * 0.9 + loss_val * 0.1
#         
#         return total_loss / len(self.train_loader)
# 
# def train_with_robust_loss(model, train_loader, val_loader, device):
#     """
#     Training with robust loss functions
#     """
#     trainer = RobustDDGTrainer(model, train_loader, val_loader, device)
#     optimizer = torch.optim.AdamW(model.parameters(), lr=2e-5)
#     
#     for epoch in range(50):
#         train_loss = trainer.train_epoch_with_analysis(optimizer, criterion_type='huber')
#         
#         # Validation
#         model.eval()
#         val_loss = 0
