...
class ModelConfig:
    ...
    num_hidden_layers: int = 4  # Reduced for debugging
    num_attention_heads: int = 4  # Reduced for debugging
    intermediate_size: int = 512  # Reduced for debugging
    max_position_embeddings: int = 64  # Reduced for debugging
    dropout: float = 0.1
    activation: str = "gelu"
    regression_head_size: int = 256
...
class ProtoBERTEmbeddings(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, token_type_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # Token embeddings
        token_embeds = self.token_embeddings(input_ids.to('cpu'))
        ...
        token_type_embeds = self.token_type_embeddings(token_type_ids.to('cpu'))
...
class ProtoBERTSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, 
        ...
        if attention_mask is not None:
            ...
            attention_scores = attention_scores.to('cpu') + attention_mask.to('cpu')
...
def train_epoch_ddg(model, dataloader, optimizer, device, scheduler=None, 
    ...
    """Enhanced training epoch with debugging info"""
    model.train()
    total_loss = 0
    all_predictions = []
    all_labels = []
...
def train_model_ddg(model, train_dataloader, val_dataloader, config: TrainingConfig):
    """Enhanced training with debugging and early stopping"""
...
def main():
    ...
    with torch.no_grad():
        ...
        input_ids = sample_batch['input_ids'][:5].to(training_config.device)
        attention_mask = sample_batch['attention_mask'][:5].to(training_config.device)
        ...
        actual_ddg = sample_batch['ddg_labels'][:5].numpy()
