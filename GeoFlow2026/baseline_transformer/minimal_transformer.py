...
# --- HYPERPARAMETERS ---
vocab_size = 1000  # Small vocab for toy example
...
# Input: "A B C" -> Output: "A B C" (shifted by 1 for causal LM)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        Q = self.W_q(x).view(batch_size, seq_len, self.nhead, self.head_dim).transpose(1, 2) # (B, nh, S, hd)
        ...
        scores = torch.matmul(Q, K.transpose(-2, -1)) / (self.head_dim ** 0.5) # (B, nh, S, S)
        if mask is not None:
            scores.masked_fill_(mask == 0, float('-inf'))
        attn_weights = F.softmax(scores, dim=-1) # (B, nh, S, S)
...
criterion = nn.CrossEntropyLoss(ignore_index=0) # Ignore padding/start token 0
